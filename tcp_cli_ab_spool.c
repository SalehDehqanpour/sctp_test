...
//short pool_num=-1;
int conn_pool=-1; //socket functions return a non-negative value. '-1' means free pool
...
if (assigned == 0){	 
  ...
  //  puts(server_reply);
	...
	if (conn_pool==-1) conn_pool= socket_desc;
	else close(socket_desc);
   return NULL;
}