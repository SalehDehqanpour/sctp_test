...
unsigned long c;
unsigned int reqs[MAX_THREADS]; // For our code to be thread-safe, I have declared a separeate variable for each thread so as to using mutex scarcely.
unsigned int resps[MAX_THREADS];
unsigned int fails_total[MAX_THREADS];
unsigned int fails_timeout[MAX_THREADS];
...
int main (int argc, char* argv[])
{
	...
	pthread_t reques_handle;
    long j;
    GET_TIME(t1);
	
	pthread_create( &reques_handle, NULL, req_handle, (void*)N );
    for (j=0; j<c; j++)  {
	...
	pthread_join(reques_handle, NULL);
    GET_TIME(t2);
    printf("\nResult = %g #/sec \n", N/(t2-t1));
}